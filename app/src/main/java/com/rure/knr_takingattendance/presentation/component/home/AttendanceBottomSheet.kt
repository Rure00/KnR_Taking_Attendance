...
fun AttendanceBottomSheet(
    ...
    onHide: (AttendanceState) -> Unit
) {
    val sheetState = rememberModalBottomSheetState()
    val coroutineScope = rememberCoroutineScope()

    val selectedState = remember {
        mutableStateOf(participation.attendanceStatus)
    }
    ...
    ModalBottomSheet(
        ...
        onDismissRequest = { onHide(selectedState.value) }
    ) {
        View(participation) {
            coroutineScope.launch {
                selectedState.value = it
                onHide(selectedState.value)
...
@Composable
private fun View(participation: MemberParticipation, onSelect: (AttendanceState) -> Unit) {
